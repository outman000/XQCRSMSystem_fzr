...
{
    public partial class UserEnterList : System.Web.UI.Page
    {
        ...
        protected void GridLoad()
        {
            ...
            string sql = "select a.*,b.Mobile,c.name as Community,d.name as Subdistrict,b.Address,b.ResidentOrVisitor "+
                    ...
                    " on a.SubdistrictID = d.code where 1=1 ";
            List<SqlParameter> paras = new List<SqlParameter>();
            //社区
            if (ddlCommunity.SelectedValue != "99")
            {
                sql += " and  a.CommunityID=@CommunityID ";
            ...
            {
                sql += " and a.SubdistrictID=@SubdistrictID ";
                SqlParameter paran = new SqlParameter("@SubdistrictID", ddlSubdistrict.SelectedValue.Trim());
                paras.Add(paran);
            }
            //常驻 或 访客
            if (ddlType.SelectedValue!="99")
            {
                sql += " and b.ResidentOrVisitor=@ResidentOrVisitor ";
                SqlParameter paran = new SqlParameter("@ResidentOrVisitor", ddlType.SelectedValue.Trim());
                paras.Add(paran);
            }
            //反馈结果
            if (ddlStatus.SelectedValue!="99")
            {
                ...
                SqlParameter paran = new SqlParameter("@Status", ddlStatus.SelectedValue.Trim());
                paras.Add(paran);
            }
            //进入时间 stime
            if (!String.IsNullOrEmpty(tbStime.Text.Trim()))
            {
                sql += " and convert(nvarchar (50),a.CreateDate,23)>= @sDate ";
            ...
            {
                sql += " and convert(nvarchar (50),a.CreateDate,23)<= @eDate ";

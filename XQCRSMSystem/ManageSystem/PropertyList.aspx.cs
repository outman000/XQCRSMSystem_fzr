...
namespace XQCRSMSystem.ManageSystem
{
    public partial class PropertyList : System.Web.UI.Page
    {
        ...
        protected void GridLoad()
        {
            ...
            {
                sql += "  and ( UserName like @keys or UserID like @keys or Mobile like @keys   ) ";
        ...
        //重置密码
        protected void lSetPW_Click(object sender, EventArgs e)
        {
            int rowIndex = ((GridViewRow)((LinkButton)sender).NamingContainer).RowIndex;
            //该行对应的ID
            string id = GridView1.DataKeys[rowIndex].Values["ID"].ToString();
            string update = " update XQCR_Property_Info set Password='1qaz2wsx' where id='"+ id + "'";
            int i = DbHelperSQL.ExecuteSql(update);
            if (i > 0)
            {
                Response.Write("<script language='javascript'>alert('重置成功！');</script>");
            }
        }
